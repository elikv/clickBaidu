$(document).ready(function (){
    ...
    $('#search').on('click', function (){
        ...
        $.ajax({
        	url:'//123.206.206.111/baiduClick/long2ShortUrl?url=' + link,
        	success:function (data) {
                if (data){
                	console.log(data);
                    link = data.urls[0].url_short;
                    console.log(data.urls[0].url_short);
                }
                $('#link').show();
...
//        $.get('//123.206.206.111/long2ShortUrl?url=' + link, {dataType:"jsonp"} ,function (data) {
//            if (data){
//                link = data['url_short'];
//            }
//            $('#link').show();
